...
def set_blender_path():
    currdir = os.getcwd()
    root = tkinter.Tk()
    tempdir = filedialog.askopenfilename(parent=root, title='Select blender install location')
    if len(tempdir) == 0:
        sys.exit()
    tempdir = (tempdir + "/Contents/MacOS/Blender")
    root.destroy()
    list_of_lines = ["", "", ""]
    list_of_lines[0] = (str(tempdir))
    list_of_lines[1] = ("\nFalse")
    list_of_lines[2] = ("\nDark")
...
def main(files, args):   
    ...
    Title =     [
                    [sg.Menu(MenuLayout)],
                    [sg.Text("Filename", size=(62)), sg.VerticalSeparator(), sg.Text("Arguments", size=(36))], 
    ...
    lineNum = 0
    for blend in files:
        line = [[sg.InputText(default_text=(blend), readonly=True, disabled_readonly_background_color=(disabledColor), p=(0,1), border_width=(3), size=(64)), sg.InputText(default_text=(args[lineNum]), readonly=True, disabled_readonly_background_color=(disabledColor), p=(0,1), border_width=(3), size=(20)), sg.Button("Edit", p=(1,1), k=str(str(lineNum) + "e"), border_width="0", button_color=(buttonTextColor,sg.theme_background_color()), mouseover_colors=(buttonTextColor, sg.theme_background_color()), image_source=(ButtonEditIMG)), sg.Button("Remove", p=(1,1), k=str(str(lineNum) + "r"), border_width="0", button_color=(buttonTextColor,sg.theme_background_color()), mouseover_colors=(buttonTextColor, sg.theme_background_color()), image_source=(ButtonRenderIMG))]]
        Files.extend(line)
        lineNum = lineNum + 1

    Preferences =[
                    [sg.HorizontalSeparator()],
                    [sg.Button("Add files", k="add", border_width="0", button_color=(buttonTextColor,sg.theme_background_color()), mouseover_colors=(buttonTextColor, sg.theme_background_color()), image_source=(ButtonIMG), p=(1, 10)), sg.Sizer(h_pixels=520), sg.Button("Start render", border_width="0", button_color=(buttonTextColor,sg.theme_background_color()), mouseover_colors=(buttonTextColor, sg.theme_background_color()), image_source=(ButtonRenderIMG), p=(1, 10)), sg.Button("Save command", border_width="0", button_color=(buttonTextColor,sg.theme_background_color()), mouseover_colors=(buttonTextColor, sg.theme_background_color()), image_source=(ButtonRenderIMG), p=(1, 10))]
                ]
    Console =   [
...
def PreferencesWindow():
    ...
    Preferences =    [
                        [sg.Text("Blender path:")],
                        [sg.InputText(default_text=(blenderpath), k="pathBox", readonly=True, disabled_readonly_background_color=(disabledColor), p=(0,1), border_width=(3), size=(50)), sg.Button("Edit", border_width="0", button_color=(buttonTextColor,sg.theme_background_color()), mouseover_colors=(buttonTextColor, sg.theme_background_color()), image_source=(ButtonEditIMG))],
                        [sg.Sizer(1, 25)],
                        [sg.Text("Theme:")],
    ...
    while True:
        ...
        if event == "Edit":
            temppath = editBlenderPath()
            if temppath != None:
                window["pathBox"].update(temppath)
                window.refresh()
        if event == "Cancel":
            window.close()
            return
        if event == "Apply":
            temppath = values["pathBox"]
            newTheme = values["themeSelect"]
            status = updatePrefs(temppath, newTheme)
            if status == "updated":
                window.close()
                return
            if status == "cancelled":
                ...
                window.refresh()
def updatePrefs(tempBlenderPath, newTheme):
    lol = []
    f = open("BlenderRendererThingPrefs", "r")
    lol = f.readlines()
    f.close
    lol[0] = (tempBlenderPath)
    lol[2] = newTheme
...
def editBlenderPath():
    currdir = os.getcwd()
    root = tkinter.Tk()
    tempdir = filedialog.askopenfilename(parent=root, initialdir="C:/", title='Select blender install location')
    root.destroy()
    if len(tempdir) == 0:
        return
    tempdir = (tempdir + "/Contents/MacOS/blender\n")
...
def generateCommand(files, args, path=os.getcwd()+"/"+"Render.command"):
    ...
    lines = ["echo off\n", blenderpath[:size-1]," -b " +  Render_command, "\necho Render Finished!"]
